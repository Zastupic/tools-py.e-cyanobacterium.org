...
#    if current_user.is_authenticated:
    if request.method == "POST": 
        ...
        file_MULTI_COLOR_PAM = file_Aquapen = Summary_file = param_all = QYALL = ETRALL = FTALL = FMALL = NPQALL = FITALL = QP = QN = pd.DataFrame()   
        F0 = FM = FMM = ETRMPOT = ALPHA = BETA = ETRMAX = IK = IB = ETRMAX_FROM_ALPHA_BETA = pd.Series()
        ALLOWED_EXTENSIONS_MULTI_COLOR_PAM = set(['.csv', '.CSV'])
        ALLOWED_EXTENSIONS_AQUAPEN = set(['.txt']) 
        files_extensions = set()
        upload_folder = UPLOAD_FOLDER
        plot_from_memory = parameters_from_memory = fluorescence = ()   
        light_intensities = []
...
#### 1) Make .zip file with example files for download      
#### 2) set ETR_max to initial condidions AND boundaries 
                                ...
                                for i in range(len(ETRALL.columns)):
                                    if i > 0:
                                        ...
                                        Ib = ETRMAX.iloc[i-1] / beta # Calculate Ib = ETRmax/beta
                                ...
                                # Decorate fig 1
                                fig_1.set_title("Light curves (raw data)") 
                                ...
                                fig_1.set_ylabel('r.u.') 
                                # Sub-plot BETA 
                                fig_2 = fig.add_subplot(3, 4, 2)                  
                                BETA.plot.bar(xticks=[], color=colors)
                                fig_2.set_title("Beta")
                                fig_2.set_ylabel('r.u.') # e$^{-}$ m$^{-2}$ s$^{-1}$
                                # Sub-plot ETRm POT 
                                fig_3 = fig.add_subplot(3, 4, 3)  

##### 3) Draw legend
#                                    # Prepare the plot
#                                    for i in range(len(pd.DataFrame(ETRMAX).columns)):
#                                        # do not plot time axis
#                                        plt.bar(
#                                            ETRMAX.columns[i], # x-axis data
#                                            ETRMAX.iloc[:, i], # y-axis data
#                                            label = ETRMAX.columns[i], # Column names for legend
#                                            color=colors[i],
#                                            width = 0.5 # width of the columns
#                                            )     
#                                    fig_3.margins(x=0.42**len(ETRMAX.columns)) # space between the axes and the first and last bar
#                                    fig_3.set_xticks([]) # no X-axis values
#                                    fig_3.legend(loc='upper left', bbox_to_anchor=(1.1, 1.02)) # legend    
#                                    fig_3.set_title("ETR max measured")
#                                    fig_3.set_ylabel('r.u.')  # e$^{-}$ m$^{-2}$ s$^{-1}$

                                ETRMAX.plot.bar(xticks=[], color=colors)
        ...
        return render_template("light_curves_analysis.html",
                        ...
                        file_MULTI_COLOR_PAM = file_MULTI_COLOR_PAM,
                        file_Aquapen = file_Aquapen,
