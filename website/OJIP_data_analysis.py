...
#    if current_user.is_authenticated:
    if request.method == "POST": 
        ...
        AREAOJ = AREAJI = AREAIP = AREAOP = pd.Series()      
        ...
        F0_index = F_20us_index = F_50us_index = F_100us_index = FK_300us_index = FJ_2ms_index = FI_30_ms_index = int()
        F_50_ms_index = F_100_ms_index = F_200_ms_index = F_300_ms_index = Fm_index = Fm_time = int()
                    ...
                    for file in files:
                        ...
                        if fluorometer == 'MULTI-COLOR-PAM / Dual PAM (Heinz Walz GmbH)':
                            ...
                            if file_extension in ALLOWED_EXTENSIONS_MULTI_COLOR_PAM:
                                ...
                                OJIP_file_MULTI_COLOR_PAM = pd.read_csv(files[(file_number)], sep=';') # type: ignore
                    ...
                    if (fluorometer == 'MULTI-COLOR-PAM / Dual PAM (Heinz Walz GmbH)' and '.csv' in files_extensions) or (fluorometer == 'Aquapen' and '.txt' in files_extensions):
                        ...
                        for i in range(len(Summary_file.columns)):
                            ...
                            if i > 0:
                                ...
                                area_below_curve_O_J = np.trapz(Summary_file.iloc[:FJ_2ms_index, i], Summary_file.iloc[:FJ_2ms_index, 0]) # (y, x)
                                area_below_curve_J_I = np.trapz(Summary_file.iloc[FJ_2ms_index:FI_30_ms_index, i], Summary_file.iloc[FJ_2ms_index:FI_30_ms_index, 0]) # (y, x)                                
                                area_below_curve_I_P = np.trapz(Summary_file.iloc[FI_30_ms_index:Fm_index, i], Summary_file.iloc[FI_30_ms_index:Fm_index, 0]) # (y, x) 
                                ...
                                total_area_O_J = max(Summary_file.iloc[:FJ_2ms_index, 0]) * max(Summary_file.iloc[:Fm_index, i])
                                total_area_J_I = (max(Summary_file.iloc[FJ_2ms_index:FI_30_ms_index, 0]) - min(Summary_file.iloc[FJ_2ms_index:FI_30_ms_index, 0])) * max(Summary_file.iloc[:Fm_index, i])
                                total_area_I_P = (max(Summary_file.iloc[FI_30_ms_index:Fm_index, 0]) - min(Summary_file.iloc[FI_30_ms_index:Fm_index, 0])) * max(Summary_file.iloc[:Fm_index, i])
                                ...
                                # Get time of Fm
#                                   Fm_time = Summary_file[x_axis_time].iloc[Fm_index] # type: ignore
                        ...
                        fig_1 = fig.add_subplot(231) # https://stackoverflow.com/questions/3584805/what-does-the-argument-mean-in-fig-add-subplot111
                        ...
                        FM_list = pd.DataFrame([FM])# FM to df, needed for legend
                        # Sub-plot F0 
                        fig_1 = fig.add_subplot(7, 5, 1)                  
                        F0.plot.bar(xticks=[], color=colors)
                        fig_1.set_title("F$_{in}$")
                        fig_1.set_ylabel(y_axis_unit)
                        # Sub-plot FJ 
                        fig_2 = fig.add_subplot(7, 5, 2)                  
                        FJ.plot.bar(xticks=[], color=colors)
                        fig_2.set_title("F$_{J}$ (2 ms)")
                        # Sub-plot FI 
                        fig_3 = fig.add_subplot(7, 5, 3) 
                        FI.plot.bar(xticks=[],color=colors)
                        fig_3.set_title("F$_{I}$ (30 ms)")
                        # Sub-plot FM 
                        fig_5 = fig.add_subplot(7, 5, 4)   
                        # Prepare the plot
                        for i in range(len(FM_list.columns)):
                            # do not plot time axis
                            plt.bar(
                                FM_list.columns[i], # x-axis data
                                FM_list.iloc[:, i], # y-axis data
                                label = FM_list.columns[i], # Column names for legend
                                color=colors[i],
                                width = 0.5 # width of the columns
                                )  
                        fig_5.margins(x=0.42**len(FM_list.columns)) # space between the axes and the first and last bar
                        fig_5.set_title("F$_{max}$")
                        fig_5.set_xticks([]) # no X-axis values
                        fig_5.legend(loc='upper left', bbox_to_anchor=(1.1, 1.02)) # legend
                        # Sub-plot  
                        fig_25 = fig.add_subplot(7, 5, 6)                  
                        OJ.plot.bar(xticks=[], color=colors)
                        fig_25.set_title("A$_{0-J}$")
                        fig_25.set_ylabel(" Fluorescence intensity (r.u.)")
                        # Sub-plot  
                        fig_27 = fig.add_subplot(7, 5, 7)                  
                        JI.plot.bar(xticks=[], color=colors)
                        fig_27.set_title("A$_{J-I}$")
                        # Sub-plot  
                        fig_29 = fig.add_subplot(7, 5, 8)                  
                        IP.plot.bar(xticks=[], color=colors)
                        fig_29.set_title("A$_{I-P}$")
                        # Sub-plot 
                        fig_6 = fig.add_subplot(7, 5, 9)                  
                        VJ.plot.bar(xticks=[], color=colors)
                        fig_6.set_title("V$_{J}$")
                        fig_6.set_ylabel("r.u.")
                        # Sub-plot VI 
                        fig_7 = fig.add_subplot(7, 5, 11)                  
                        VI.plot.bar(xticks=[], color=colors)
                        fig_7.set_title("V$_{I}$")
                        fig_7.set_ylabel("r.u.")
                        # Sub-plot M0 
                        fig_9 = fig.add_subplot(7, 5, 12)                  
                        M0.plot.bar(xticks=[], color=colors)
                        fig_9.set_title("M$_{0}$")
                        # Sub-plot PSIE0 
                        fig_10 = fig.add_subplot(7, 5, 13)                  
                        PSIE0.plot.bar(xticks=[], color=colors)
                        fig_10.set_title("ψE$_{0}$")
                        # Sub-plot PSIR0 
                        fig_11 = fig.add_subplot(7, 5, 14)                  
                        PSIR0.plot.bar(xticks=[], color=colors)
                        fig_11.set_title("ψR$_{0}$")
                        # Sub-plot DELTAR0 
                        fig_13 = fig.add_subplot(7, 5, 16)                  
                        DELTAR0.plot.bar(xticks=[], color=colors)
                        fig_13.set_title("δR$_{0}$")
                        fig_13.set_ylabel("r.u.")
                        # Sub-plot PHIP0 
                        fig_14 = fig.add_subplot(7, 5, 17)                  
                        FVFM.plot.bar(xticks=[], color=colors)
                        fig_14.set_title("φE$_{0}$ (F$_{v}$ / F$_{max}$)")
                        # Sub-plot PHIE0 
                        fig_15 = fig.add_subplot(7, 5, 18)                  
                        PHIE0.plot.bar(xticks=[], color=colors)
                        fig_15.set_title("φE$_{0}$")
                        # Sub-plot PHIR0 
                        fig_17 = fig.add_subplot(7, 5, 19)                  
                        PHIR0.plot.bar(xticks=[], color=colors)
                        fig_17.set_title("φR$_{0}$")
                        # Sub-plot ABSRC 
                        fig_18 = fig.add_subplot(7, 5, 21)                  
                        ABSRC.plot.bar(xticks=[], color=colors)
                        fig_18.set_title("ABS/RC")
                        fig_18.set_ylabel("r.u.")
                        # Sub-plot TR0RC 
                        fig_19 = fig.add_subplot(7, 5, 22)                  
                        TR0RC.plot.bar(xticks=[], color=colors)
                        fig_19.set_title("TR0/RC")
                        # Sub-plot ET0RC 
                        fig_21 = fig.add_subplot(7, 5, 23)                  
                        ET0RC.plot.bar(xticks=[], color=colors)
                        fig_21.set_title("ET0/RC")
                        # Sub-plot RE0RC 
                        fig_22 = fig.add_subplot(7, 5, 24)                  
                        RE0RC.plot.bar(xticks=[], color=colors)
                        fig_22.set_title("RE0/RC")
                        # Sub-plot DI0RC 
                        fig_23 = fig.add_subplot(7, 5, 26)                  
                        DI0RC.plot.bar(xticks=[], color=colors)
                        fig_23.set_title("DI0/RC")
                        fig_23.set_ylabel("r.u.")
                        # Sub-plot area above curve O-J 
                        fig_26 = fig.add_subplot(7, 5, 27)                  
                        AREAOJ.plot.bar(xticks=[], color=colors)
                        fig_26.set_title("Area$_{0-J}$")
                        # Sub-plot 
                        fig_27 = fig.add_subplot(7, 5, 28)                  
                        AREAJI.plot.bar(xticks=[], color=colors)
                        fig_27.set_title("Area$_{J-I}$")
                        # Sub-plot 
                        fig_28 = fig.add_subplot(7, 5, 29)                  
                        AREAIP.plot.bar(xticks=[], color=colors)
                        fig_28.set_title("Area$_{I-P}$")
                        # Sub-plot 
                        fig_28 = fig.add_subplot(7, 5, 31)                  
                        AREAOP.plot.bar(xticks=[], color=colors)
                        fig_28.set_title("Area$_{(0-P)}$")
                        fig_28.set_ylabel("r.u.")
                        # Sub-plot 
                        fig_29 = fig.add_subplot(7, 5, 32)                  
                        SM.plot.bar(xticks=[], color=colors)
                        fig_29.set_title("Normalized area S$_{m}$")
                        # Sub-plot 
                        fig_30 = fig.add_subplot(7, 5, 33)                  
                        N.plot.bar(xticks=[], color=colors)
                        fig_30.set_title("N (turn-over number Q$_{A}$)")
                        # saving scatter plot to memory
                       ...
                       # prepare DF with parameters
                        ...
                        # name columns
                        OJIP_param_all.columns = ['Fin', 'FK', 'FJ', 'FI', 'Fmax', 'Amplitude(0-J)', 'Amplitude(J-I)', 'Amplitude(I-P)', 'VJ', 'VI', 'M0',
                                                 'ψE0', 'ψR0', 'δR0', 'ψP0 (Fv/Fm)','φE0', 'φR0', 'ABS/RC', 'TR0/RC', 'ET0/RC', 'RE0/RC', 'DI0/RC', 
                                                 'Complementary area O-J', 'Complementary area J-I', 'Complementary area I-P', 'Complementary area (O-P)',
